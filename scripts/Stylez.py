    ...
    try:
        for root, dirs, _ in os.walk(styles_dir):
            for directory in dirs:
                ...
                if subfolder_name.lower() not in [c.lower() for c in categories_list]:
...
def filename_check(folder, filename):
    if filename is None or len(filename) == 0:
        warning_msg = """<p id="style_filename_check" style="color:red;">please add a file name</p>"""
    else:
        save_folder_path = os.path.join(extension_path, "styles", folder)
        json_file_path = os.path.join(save_folder_path, filename + ".json")
        if os.path.exists(json_file_path):
            warning_msg = f"""<p id="style_filename_check" style="color:red;">Overwrite!! File Already Exists In '{folder}'</p>"""
        else:
            warning_msg = """<p id="style_filename_check" style="color:green;">Filename Is Valid</p>"""
    return gr.update(value=warning_msg)
...
def oldstyles(value):
    with open(config_json, "r") as json_file:
        data = json.load(json_file)
        if data["hide_old_styles"] is True:
            ...
            with gr.TabItem(label="Styles", elem_id="styles_libary"):
                with gr.Column():
                            ...
                            with gr.TabItem(label="CivitAI"):
                                with gr.Row():
                                    with gr.Column(elem_id="civit_tags_column"):
                                        nsfwlvl = gr.Dropdown(label="NSFW:", choices=["None", "Soft", "Mature", "X"], value="None", elem_id="civit_nsfwfilter", elem_classes="dropdown styles_dropdown", scale=1)
                                        ...
                                        periodcivit = gr.Dropdown(label="Period:", choices=["AllTime", "Year", "Month", "Week", "Day"], value="AllTime", elem_id="civit_periodfilter", elem_classes="dropdown styles_dropdown", scale=1)
                                    with gr.Column():
                                        with gr.Row(elem_id="style_search_search"):
                                            ...
                                            civitAI_refresh = gr.Button(refresh_symbol, elem_id="style_refresh", elem_classes="tool")
                    ...
                    with gr.Row(elem_id="stylesPreviewRow"):
                        ...
                        gr.Checkbox(value=True, label="Hover Over Preview", elem_id="HoverOverStyle_preview", elem_classes="styles_checkbox checkbox")
                        oldstylesCB = gr.Checkbox(value=hideoldstyles, label="Hide Styles Bar", elem_id="hide_default_styles", elem_classes="styles_checkbox checkbox", interactive=True)
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        with gr.Row():
                            ...
                            style_savefolder_txt = gr.Dropdown(label="Save Folder (Type To Create A New Folder):", value="Styles", choices=generate_styles_and_tags[2], elem_id="style_savefolder_txt", elem_classes="dropdown", allow_custom_value=True)
